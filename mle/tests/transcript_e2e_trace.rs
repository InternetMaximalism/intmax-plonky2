...
fn solidity_terminal_golden(trace: &[TraceCheckpoint]) -> String {
    let terminal = trace.last().expect("outer transcript must be nonempty");
    format!(
        concat!(
            ...
            "bytes32 constant PACKED_V1_GOLDEN_TERMINAL_DIGEST = {};\n"
        ),
...
fn test_v1_transcript_golden_trace() {
    ...
    let solidity_golden_path = concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/contracts/test/TranscriptV1Golden.sol"
    );
    let generated_solidity = solidity_terminal_golden(&trace);
    if std::env::var_os("MLE_WRITE_TRANSCRIPT_TRACE").as_deref() == Some(std::ffi::OsStr::new("1"))
    {
        ...
        std::fs::write(solidity_golden_path, &generated_solidity).unwrap();
    }
    assert_eq!(
        std::fs::read_to_string(solidity_golden_path).unwrap(),
        generated_solidity,
        "Solidity terminal golden drift; regenerate with MLE_WRITE_TRANSCRIPT_TRACE=1"
    );
